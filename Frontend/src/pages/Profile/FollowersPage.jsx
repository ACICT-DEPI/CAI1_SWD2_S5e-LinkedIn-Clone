      ...
      <div className="bg-white rounded-lg shadow-md p-5 w-3/4 md:w-1/2 mt-24">
        ...
        {followers.length === 0 ? (
          ...
          followers.map((follower, index) => (
            <div
              key={index}
              className="flex justify-between items-center my-4 border-b pb-2"
            >
              <div className="flex items-center gap-3">
                <img
                  src={
                    follower.profilePicture || "https://via.placeholder.com/40"
                ...
                <div>
                  <Link to={`/profile/${follower._id}`}>
                    <h4 className="font-semibold text-linkedinDarkGray hover:text-LinkHoverBlue hover:underline">
                      {`${follower.firstName} ${follower.lastName}` ||
                        "Anonymous User"}
                    </h4>
                  </Link>
                  <p className="text-sm text-linkedinGray">
                    {follower.headline || "No headline provided"}
                  </p>
                </div>
              </div>
            </div>
